    ...
    <div class="login-main-div">
        <form @submit.prevent="handleSubmit">
            ...
            <div class="mb-3">
                ...
                <p>That's You Entered While Signup</p>
            </div>
            <div class="mb-3">
                <label for="emailPassword1" class="form-label">Create A New Password</label>
                <input type="password" required class="form-control" id="exampleInputPassword1" v-model="password">
            </div>
            <div class="mb-3">
                <label for="emailPassword2" class="form-label">Create A Confirm Password</label>
                <input type="text" required class="form-control" id="exampleInputPassword2" v-model="confirm_password">
            </div>
            <button type="submit" class="btn btn-success">Submit</button>
            <p>remember your password ? <router-link to="/login" class="link">click here</router-link></p>
        </form>
    </div>
</template>
    ...
    methods: {
        async handleSubmit() {
            if (this.password !== this.confirm_password) {
                ...
                if (message !== "user updated success") {
